...
class OpenAIService(LLMService):
    ...
    async def generate(self, prompt: str, **kwargs) -> str:
        ...
        try:
            ...
            logger.error(f"OpenAI API error: {e!s}", exc_info=True)
            raise Exception(f"OpenAI API error: {e!s}") from e
...
class TransformersService(LLMService):
    ...
    def _load_model(self):
        ...
        try:
            ...
            logger.error(f"Failed to load model {self.model_name}: {e!s}", exc_info=True)
    ...
    async def generate(self, prompt: str, **kwargs) -> str:
        ...
        try:
            ...
            logger.error(f"Transformers generation error: {e!s}", exc_info=True)
            raise Exception(f"Transformers generation error: {e!s}") from e
...
class LLMServiceFactory:
    ...
    def create_service(provider: LLMProvider, model: str) -> LLMService:
        ...
        try:
            ...
            logger.error(f"Failed to create LLM service: {e!s}", exc_info=True)
            raise
