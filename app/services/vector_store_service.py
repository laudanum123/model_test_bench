...
class VectorStoreService(ABC):
    ...
    @abstractmethod
    async def add_documents(self, documents: list[str], metadata: list[dict[str, Any]] | None = None) -> list[str]:
...
class ChromaService(VectorStoreService):
    ...
    async def add_documents(self, documents: list[str], metadata: list[dict[str, Any]] | None = None) -> list[str]:
        try:
            ...
            raise Exception(f"ChromaDB add documents error: {e!s}") from e
    ...
    async def search(self, query_embedding: list[float], top_k: int = 5) -> list[dict[str, Any]]:
        ...
        try:
            ...
            raise Exception(f"ChromaDB search error: {e!s}") from e
    ...
    async def clear(self):
        try:
            ...
            raise Exception(f"ChromaDB clear error: {e!s}") from e
...
class FAISSService(VectorStoreService):
    ...
    async def add_documents(self, documents: list[str], metadata: list[dict[str, Any]] | None = None) -> list[str]:
        try:
            ...
            raise Exception(f"FAISS add documents error: {e!s}") from e
    ...
    async def search(self, query_embedding: list[float], top_k: int = 5) -> list[dict[str, Any]]:
        try:
            ...
            # Format results
            results = []
            for _, (distance, idx) in enumerate(zip(distances[0], indices[0], strict=False)):
            ...
            raise Exception(f"FAISS search error: {e!s}") from e
    ...
    async def clear(self):
        try:
            ...
            raise Exception(f"FAISS clear error: {e!s}") from e
    ...
    async def update_embeddings(self, embeddings: list[list[float]]):
        ...
        try:
            ...
            raise Exception(f"FAISS update embeddings error: {e!s}") from e
