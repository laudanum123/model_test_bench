...
Provide only the numerical score (0.0-1.0):"""
            ...
            print(f"Error in answer relevance evaluation: {e!s}")
            # Fallback to embedding similarity
            return await self._calculate_similarity(reference_answer, generated_answer)
...
Provide only the numerical score (0.0-1.0):"""
            ...
            print(f"Error in retrieval relevance evaluation: {e!s}")
            # Fallback to embedding similarity
            return await self._calculate_similarity(question, combined_chunks)
...
Provide only the numerical score (0.0-1.0):"""
            ...
            print(f"Error in chunk overlap evaluation: {e!s}")
            # Fallback to embedding similarity
            return await self._calculate_similarity(reference_answer, combined_chunks)
    ...
    async def _calculate_similarity(self, text1: str, text2: str) -> float:
        ...
        try:
            ...
            print(f"Error calculating similarity: {e!s}")
            return 0.0
    ...
    async def evaluate_single_result(self, question: str, reference_answer: str,
        ...
        # Handle any exceptions and ensure scores are floats
        score_0 = scores[0]
        answer_relevance = 0.0 if isinstance(score_0, BaseException) else float(score_0)

        score_1 = scores[1]
        retrieval_relevance = 0.0 if isinstance(score_1, BaseException) else float(score_1)

        score_2 = scores[2]
        chunk_overlap = 0.0 if isinstance(score_2, BaseException) else float(score_2)
